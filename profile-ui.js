...
const api={profile:null,avatarUrl:null,onlineIds:new Set(),channel:null,client:null,session:null,notifications:[],notifChannel:null,refresh,loadNotifications};
...
function addStyle(){
 ...
 .nettoBellWrap,.nettoUserWrap{position:relative}
 ...
 .nettoNavBtn{width:100%;display:grid;grid-template-columns:34px minmax(0,1fr);gap:9px;align-items:center;text-align:left;border:0;background:transparent;border-radius:11px;padding:8px 9px;cursor:pointer}.nettoNavBtn:hover{background:#f4f5f7}.nettoNavBtn>span:first-child{width:32px;height:32px;border-radius:9px;background:#f0f1f3;display:grid;place-items:center;color:#e54726;font-weight:850;overflow:hidden}.nettoNavBtn>span:first-child img{width:100%;height:100%;display:block;object-fit:cover}.nettoNavBtn strong{display:block;font-size:11px}.nettoNavBtn small{display:block;font-size:8.5px;color:#8b8e94;margin-top:1px}.nettoLogout{margin-top:5px;border-top:1px solid #eceef1;border-radius:0 0 10px 10px;color:#a52218}
 ...
 .nettoNotifItem.manual_edit{background:#f2f6ff;border-color:#dce6fb}.nettoNotifItem.manual_edit .nettoNotifIcon{background:#e2ebff;color:#335fba}.nettoNotifItem.import_new{background:#eff9f2;border-color:#d5eedc}.nettoNotifItem.import_new .nettoNotifIcon{background:#dff3e5;color:#188443}.nettoNotifItem.import_replace{background:#fff6ea;border-color:#ffe0b5}.nettoNotifItem.import_replace .nettoNotifIcon{background:#ffead0;color:#c96a00}.nettoNotifItem.reset_day,.nettoNotifItem.reset_week{background:#fff1ef;border-color:#ffd8d1}.nettoNotifItem.reset_day .nettoNotifIcon,.nettoNotifItem.reset_week .nettoNotifIcon{background:#ffe1dc;color:#bd3022}
 :root[data-theme="dark"] .nettoBackBtn{background:#24262b;border-color:#3a3d43;color:#f2f3f4}:root[data-theme="dark"] .nettoBackBtn:hover{background:#2c2f35}
 :root[data-theme="dark"] .nettoBellBtn,:root[data-theme="dark"] .nettoUserBtn{background:#202226;border-color:#373a40;color:#f5f5f5}:root[data-theme="dark"] .nettoBellBtn svg{fill:#eee}:root[data-theme="dark"] .nettoNotifBadge{border-color:#202226}:root[data-theme="dark"] .nettoDrop{background:#1c1e22f8;border-color:#383b41;color:#f5f5f5}:root[data-theme="dark"] .nettoNavBtn:hover{background:#292b30}:root[data-theme="dark"] .nettoNavBtn>span:first-child{background:#302421}:root[data-theme="dark"] .nettoNotifHead,:root[data-theme="dark"] .nettoUserHead{border-color:#34363b}:root[data-theme="dark"] .nettoNotifBody span{color:#bbb}:root[data-theme="dark"] .nettoNotifItem.manual_edit{background:#182131;border-color:#293b5a}:root[data-theme="dark"] .nettoNotifItem.import_new{background:#17251c;border-color:#284532}:root[data-theme="dark"] .nettoNotifItem.import_replace{background:#2b2317;border-color:#4a3920}:root[data-theme="dark"] .nettoNotifItem.reset_day,:root[data-theme="dark"] .nettoNotifItem.reset_week{background:#2d1c1b;border-color:#4d2b27}
...
function changeTheme(){if(typeof window.toggleTheme==='function'){window.toggleTheme({stopPropagation(){}});return}const next=currentTheme()==='dark'?'light':'dark';document.documentElement.dataset.theme=next;try{localStorage.setItem('nettoTheme',next)}catch(_){};updateThemeText()}
...
function buildGlobalHeader(){
 if(document.getElementById('nettoGlobalTools')||!api.profile)return;bindHomeMark();
 const top=findHeaderTop();if(!top)return;
 ...
 wrap.innerHTML='<div class="nettoBellWrap"><button id="nettoBellBtn" class="nettoBellBtn" aria-label="Notifications" aria-expanded="false"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 22a2.55 2.55 0 0 0 2.45-1.85h-4.9A2.55 2.55 0 0 0 12 22Zm7-5.1-1.75-2.05V9.5A5.26 5.26 0 0 0 13 4.34V3a1 1 0 1 0-2 0v1.34A5.26 5.26 0 0 0 6.75 9.5v5.35L5 16.9V18h14v-1.1Z"/></svg><b id="nettoNotifBadge" class="nettoNotifBadge hidden">0</b></button><div id="nettoNotifDrop" class="nettoDrop nettoNotifDrop hidden"><div class="nettoNotifHead"><strong>Notifications</strong><div class="nettoNotifHeadActions"><button id="nettoMarkRead">Tout lire</button><button id="nettoDeleteAll">Tout supprimer</button></div></div><div id="nettoNotifList" class="nettoNotifList"><div class="nettoNotifEmpty">Chargement…</div></div></div></div><div class="nettoUserWrap"><button id="nettoUserBtn" class="nettoUserBtn" aria-expanded="false"><span id="nettoTopAvatar" class="nettoTopAvatar">U</span><span class="nettoUserText"><strong>'+esc(name)+'</strong><small>'+esc(role)+'</small></span><span class="nettoChevron">⌄</span></button><div id="nettoUserDrop" class="nettoDrop hidden"><div class="nettoUserHead"><span id="nettoMenuAvatar" class="nettoTopAvatar">U</span><span><strong>'+esc(name)+'</strong><small>'+esc(role)+'</small></span></div>'+makeButton('⌂','Accueil','Retour au portail','home.html')+makeButton('☺','Mon profil','Profil et notifications','profile.html')+(p.role==='admin'?makeButton('✦','Défis & Boutique','Missions, boutique et personnalisation','rewards.html'):'')+makeButton('<img src="assets/logo-stock.svg" alt="">','Stock F&L','Gestion du stock','index.html')+makeButton('<img src="assets/logo-planning.svg" alt="">','Planning','Horaires de l’équipe','planning.html')+makeButton('<img src="assets/logo-equipe.svg" alt="">','Équipe','Messagerie interne','chat.html')+makeButton('<img src="assets/logo-article.svg" alt="">','Fiches articles','Référentiel articles','articles.html')+(p.role==='admin'?makeButton('<img src="assets/logo-boulangerie.svg" alt="">','Boulangerie','Stock • Consulter • Gestion','bakery.html')+makeButton('◫','Vue test','Stock groupé par familles','stock-test.html')+makeButton('♙','Gestion des comptes','Utilisateurs, accès et journal','accounts.html')+makeButton('⚙','Personnalisation','Réglages du portail','settings.html'):'')+'<button id="nettoThemeBtn" class="nettoNavBtn"><span class="nettoThemeIcon">☾</span><span><strong class="nettoThemeLabel">Mode sombre</strong><small>Changer l’apparence</small></span></button><button id="nettoLogoutBtn" class="nettoNavBtn nettoLogout"><span>↪</span><span><strong>Déconnexion</strong><small>Quitter la session</small></span></button></div></div>';
 ...
 document.getElementById('nettoLogoutBtn').onclick=async()=>{sounds.play('logout');await new Promise(r=>setTimeout(r,390));await api.client.auth.signOut();location.href='index.html'};
 ...
 document.addEventListener('click',e=>{if(!e.target.closest('#nettoGlobalTools'))closeDrops()});
 document.addEventListener('keydown',e=>{if(e.key==='Escape')closeDrops()});
}
function toggleDrop(which){const n=document.getElementById('nettoNotifDrop'),u=document.getElementById('nettoUserDrop'),nb=document.getElementById('nettoBellBtn'),ub=document.getElementById('nettoUserBtn');if(which==='notifications'){const open=n.classList.contains('hidden');sounds.play(open?'menuOpen':'menuClose');n.classList.toggle('hidden',!open);u.classList.add('hidden');nb.setAttribute('aria-expanded',String(open));ub.setAttribute('aria-expanded','false');if(open)loadNotifications()}else{const open=u.classList.contains('hidden');sounds.play(open?'menuOpen':'menuClose');u.classList.toggle('hidden',!open);n.classList.add('hidden');ub.setAttribute('aria-expanded',String(open));nb.setAttribute('aria-expanded','false')}}
function closeDrops(){document.getElementById('nettoNotifDrop')?.classList.add('hidden');document.getElementById('nettoUserDrop')?.classList.add('hidden');document.getElementById('nettoBellBtn')?.setAttribute('aria-expanded','false');document.getElementById('nettoUserBtn')?.setAttribute('aria-expanded','false')}
function notificationIcon(k){return k==='import_new'?'▦':k==='import_replace'?'↻':(k==='reset_day'||k==='reset_week')?'⌫':'✎'}
...
async function refresh(){if(!api.client||!api.session)return null;const {data:p}=await api.client.from('profiles').select('display_name,role,avatar_path,status_text,profile_color').eq('id',api.session.user.id).maybeSingle();if(!p)return null;api.profile=p;api.avatarUrl=null;if(p.avatar_path){const {data:a}=await api.client.storage.from('profile-avatars').createSignedUrl(p.avatar_path,3600);api.avatarUrl=a?.signedUrl||null}document.documentElement.style.setProperty('--profile-accent',p.profile_color||'#ff5a2a');updateKnownUI();window.dispatchEvent(new CustomEvent('netto:profile',{detail:{profile:p,avatarUrl:api.avatarUrl}}));return p}
async function init(){addStyle();if(!window.supabase?.createClient)return;api.client=window.supabase.createClient(URL,KEY);const {data:{session}}=await api.client.auth.getSession();if(!session)return;api.session=session;await refresh();addBackButton();logPageView();bindHomeMark();await loadNotifications();startNotificationsRealtime();startPresence();window.addEventListener('focus',loadNotifications);document.addEventListener('visibilitychange',()=>{if(!document.hidden)loadNotifications()})}
