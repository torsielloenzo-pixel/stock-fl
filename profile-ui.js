...
const NAV_MODULES=Object.freeze([
 ...
 {id:'rewards',label:'Défis & Boutique',homeLabel:'Défis & Boutique',subtitle:'Missions et récompenses',url:'rewards.html',icon:'✦',roles:['admin'],home:true,userMenu:true,defaultHome:true,defaultUser:true,kicker:'ADMINISTRATION',description:'Créer les défis, gérer les récompenses et utiliser librement le catalogue administrateur.',action:'Ouvrir Défis & Boutique',cardClass:'rewardsCard'},
...
function moduleAllowed(module,profileOrRole){const role=typeof profileOrRole==='string'?profileOrRole:profileOrRole?.role;return !!module&&(!Array.isArray(module.roles)||module.roles.includes(role))}
function preferenceMap(profile){const p=profile?.ui_preferences;return p&&typeof p==='object'&&!Array.isArray(p)?p:{}}
function moduleVisible(area,module,profile){if(!moduleAllowed(module,profile))return false;if(area==='home'&&!module.home)return false;if(area==='user_menu'&&!module.userMenu)return false;const v=preferenceMap(profile)?.[area]?.[module.id];if(typeof v==='boolean')return v;return area==='home'?module.defaultHome!==false:module.defaultUser!==false}
function visibleModules(area,profile){return NAV_MODULES.filter(m=>moduleVisible(area,m,profile))}
function moduleIcon(module){return module?.asset?'<img src="'+esc(module.asset)+'" alt="">':esc(module?.icon||'•')}
const api={profile:null,avatarUrl:null,onlineIds:new Set(),channel:null,client:null,session:null,notifications:[],notifChannel:null,modules:NAV_MODULES,canAccess:moduleAllowed,isVisible:moduleVisible,visibleModules,refresh,loadNotifications};
