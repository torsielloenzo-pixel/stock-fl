...
function toggleMobilePreview(){const existing=document.getElementById('nettoMobilePreviewOverlay');if(existing){closeMobilePreview(true);sounds.play('menuClose');return}closeDrops();const overlay=document.createElement('div');overlay.id='nettoMobilePreviewOverlay';overlay.className='nettoMobilePreviewOverlay';overlay.innerHTML='<div class="nettoMobilePreviewDevice" role="dialog" aria-modal="true" aria-label="Aperçu mobile Nethor"><div class="nettoMobilePreviewTop"><span class="nettoMobilePreviewState">Aperçu mobile · 390 px</span><span class="nettoMobilePreviewNotch"></span><button class="nettoMobilePreviewClose" type="button" aria-label="Fermer la vision mobile">×</button></div><iframe class="nettoMobilePreviewFrame" title="Vision mobile Nethor" src="'+esc(mobilePreviewUrl())+'"></iframe></div>';overlay.addEventListener('click',e=>{if(e.target===overlay)closeMobilePreview(true)});overlay.querySelector('.nettoMobilePreviewClose').onclick=()=>closeMobilePreview(true);const frame=overlay.querySelector('.nettoMobilePreviewFrame');frame.addEventListener('load',()=>mobilePreviewNotice('Vision mobile activée · aperçu 390 px'),{once:true});frame.addEventListener('error',()=>mobilePreviewNotice('Impossible de charger l’aperçu mobile'),{once:true});document.body.appendChild(overlay);document.body.style.overflow='hidden';setMobilePreviewButton(true);mobilePreviewNotice('Ouverture de la vision mobile…');sounds.play('menuOpen')}
