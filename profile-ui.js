...
function addStyle(){
 ...
 .nettoLoginWrap,.nettoBellWrap,.nettoUserWrap{position:relative}
...
async function deleteLoginHistoryRow(id){
 ...
 const {error}=await api.client.from('login_history').delete().eq('id',id);if(error){console.warn('Suppression connexion:',error);sounds.play('error');return}
 sounds.play('delete');api.loginHistory=api.loginHistory.filter(x=>x.id!==id);renderLoginHistory();
}
async function deleteAllLoginHistory(){
 ...
 const ids=api.loginHistory.map(x=>x.id),{error}=await api.client.from('login_history').delete().in('id',ids);if(error){console.warn('Suppression historique connexions:',error);sounds.play('error');return}
