...
declare u uuid:=auth.uid(); item_ok boolean;
begin
 ...
 select exists(
   ...
   where c.id=p_item and c.kind=p_kind
     and (c.active or exists(select 1 from public.reward_inventory i where i.user_id=u and i.item_id=c.id))
 ) into item_ok;
 if not item_ok then raise exception 'Objet indisponible pour cette catégorie'; end if;
 insert into public.reward_inventory(user_id,item_id)
 select u,p_item where exists(select 1 from public.reward_catalog c where c.id=p_item and c.active)
 on conflict(user_id,item_id) do nothing;
 insert into public.reward_equipment(user_id,kind,item_id) values(u,p_kind,p_item)
